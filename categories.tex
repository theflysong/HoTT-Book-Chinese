...
\index{JingDianDe@经典的!FanChouLun@范畴论}%
\begin{enumerate}
...
We assume the reader has some basic familiarity with classical category theory.\index{JingDianDe@经典的!FanChouLun@范畴论}
Recall that whenever we write \type it denotes some universe of types, but perhaps a different one at different times; everything we say remains true for any consistent choice of universe levels\index{YuZhouCengJi@宇宙层级}.
...
\begin{defn}\label{ct:precategory}
  ...
  \item For each $a,b,c:A$, a function%
    \indexdef{FuHe@复合!of morphisms in a (pre)category}
...
\begin{defn}\label{ct:isomorphism}
  ...
  \indexdef{TongGou@同构!in a (pre)category}%
...
\begin{eg}\label{ct:orders}
  ...
  In other words, $A_0$ must be a set, and $\le$ must be antisymmetric\index{relation!FanDuiChenXing@反对称性} (if $a\le b$ and $b\le a$, then $a=b$).
...
\begin{eg}\label{ct:discrete}
  ...
  \indexdef{FanChou@范畴!LiSanDe@离散的}%
  \indexdef{LiSanDe@离散的!FanChou@范畴}%
  category on $X$.
...
\begin{eg}\label{ct:fundgpd}
  ...
  \indexdef{JiChuDe@基础的!pregroupoid}%
...
\begin{eg}\label{ct:hoprecat}
  ...
  \index{TongLun@同伦!category of types@(pre)category of types}%
\end{eg}
...
Instead, we will develop many concepts for precategories as well as categories, in order to emphasize how much better-behaved categories are, as compared both to precategories and to ordinary categories in classical\index{mathematics!JingDianDe@经典的} mathematics.
...
\begin{defn}\label{ct:nattrans}
  ...
  \indexdef{ZiRanDe@自然的!transformation}%
  \indexsee{transformation!ZiRanDe@自然的}{自然变换}%
...
\begin{defn}\label{ct:functor-precat}
  \indexdef{precategory!HanZiDe@汉字的}%
...
\begin{lem}\label{ct:natiso}
  \index{ZiRanDe@自然的!TongGou@同构}%
  \index{TongGou@同构!ZiRanDe@自然的}%
...
\begin{thm}\label{ct:functor-cat}
  \indexdef{FanChou@范畴!HanZiDe@汉字的}%
  \indexdef{HanZi@函子!FanChouDe@范畴的}%
...
\index{horizontal composition!ZiRanBianHuanDe@自然变换的}%
\index{JingDianDe@经典的!FanChouLun@范畴论}%
...
\begin{lem}\label{ct:functor-assoc}
  \index{JieHeXing@结合性!HanZiFuHeDe@函子复合的}
...
\begin{lem}\label{ct:pentagon}
  \index{JieHeXing@结合性!HanZiFuHeDe@函子复合的!coherence of}%
  \cref{ct:functor-assoc} is coherent, i.e.\ the following pentagon\index{WuBianXing, Mac Lane@五边形, Mac Lane} of equalities commutes:
...
\begin{defn}\label{ct:adjoints}
  ...
  \indexdef{left!BanSui@伴随}%
  \indexdef{BanSui@伴随!HanZi@函子}%
  \indexdef{right!BanSui@伴随}%
  \indexdef{BanSui@伴随!HanZi@函子}%
  \index{HanZi@函子!BanSui@伴随}%
  if there exists
  \begin{itemize}
...
\indexdef{BanSui@伴随!equivalence!of (pre)categories}
...
\begin{defn}\label{ct:equiv}
  ...
  \indexdef{FanChou@范畴!equivalence of}%
  \indexdef{precategory!equivalence of}%
  \index{HanZi@函子!equivalence}%
...
\begin{defn}\label{ct:full-faithful}
  ...
  \indexdef{HanZi@函子!ManZhongShiDe@满忠实的}%
  \indexdef{ManZhongShiHanZi@满忠实函子}%
\end{defn}

\begin{defn}\label{ct:split-essentially-surjective}
  We say a functor $F:A\to B$ is \define{split essentially surjective}
  \indexdef{HanZi@函子!split essentially surjective}%
...
\begin{defn}\label{ct:essentially-surjective}
  ...
  \indexdef{HanZi@函子!essentially surjective}%
  ...
  \indexsee{HanZi@函子!weak equivalence}{weak equivalence}%
...
\begin{defn}\label{ct:isocat}
  ...
  \indexdef{TongGou@同构!of (pre)categories}%
  \indexdef{FanChou@范畴!DeTongGou@的同构}%
  \indexdef{precategory!DeTongGou@的同构}%
...
\begin{defn}\label{ct:representable}
  ...
  \indexdef{HanZi@函子!representable}%
...
\begin{lem}\label{ct:adj-repr}
  ...
  \begin{enumerate}
  \item $F$ is a left adjoint\index{BanSui@伴随!HanZi@函子}.\label{item:ct:ar1}
...
\begin{defn}\label{ct:strict-category}
  ...
  \indexdef{FanChou@范畴!strict}%
  \indexdef{strict!FanChou@范畴}%
...
\begin{eg}\label{ct:galois}
  ...
  \index{Galois!Qun@群}%
  Then there is a strict category whose objects are intermediate fields $F\subseteq K\subseteq E$, and whose morphisms are field homomorphisms\index{TongTai@同态!field} which fix $F$ pointwise (but need not commute with the inclusions into $E$).
  ...
  \index{JiChuDe@基础的!theorem of Galois theory}%
...
\begin{eg}\label{ct:hilb}
  ...
  \begin{itemize}
  \item Its objects are finite-dimensional \index{finite!-WeiXiangLiangKongJian@-维向量空间} vector spaces\index{XiangLiang@向量!space} equipped with an inner product $\langle \blank,\blank\rangle$.
  \item Its morphisms are arbitrary linear maps.
    \index{HanShu@函数!linear}%
    \indexsee{linear map}{function, linear}%
  \end{itemize}
  By standard linear algebra, any linear map $f:V\to W$ between finite
  dimensional inner product spaces has a uniquely defined adjoint\index{BanSui@伴随!linear map} $\dgr f:W\to V$, characterized by $\langle f v,w\rangle = \langle v,\dgr f w\rangle$.
...
There has been a good deal of general theory developed for $\dagger$-cat\-e\-gor\-ies under classical\index{mathematics!JingDianDe@经典的}\index{JingDianDe@经典的!FanChouLun@范畴论} foundations.
...
\begin{defn}\label{ct:sig}
  ...
  \[ H_{\alpha\beta}(f).\]
    ...
    \indexdef{TongTai@同态!of structures}%
    \indexdef{structure!DeTongTai@的同态}%
    from $\alpha$ to $\beta$.
...
\begin{eg}\label{ct:sip-functor-cat}
  ...
  Given such $F$ and $G$, we define $\gamma:\hom_{B^{A_0}}(F_0, G_0)$ to be a homomorphism if it forms a natural transformation.\index{ZiRanDe@自然的!transformation}
...
\indexdef{TongTai@同态!of Omega-structures@of $\Omega$-structures}%
...
One would have to be similarly careful in classical\index{mathematics!JingDianDe@经典的}\index{JingDianDe@经典的!FanChouLun@范畴论} category theory if one wanted to avoid the axiom of choice: any time we want to define a function, we need to characterize its values uniquely somehow.
...
In classical\index{mathematics!JingDianDe@经典的}\index{JingDianDe@经典的!FanChouLun@范畴论} category theory, all one can say is that this object is specified up to unique isomorphism, but in set-theoretic foundations this is not a sufficient amount of uniqueness to give us a function without invoking \choice{}.
...
\index{FanDe@泛的!property!of Rezk completion}%
...
\indexsee{RezkWanBei@Rezk完备}{Rezk完备}%
\index{WanBei@完备!Rezk|(defstyle}%
...
\begin{proof}[Second proof]
  ...
  (The following proof is of the sort that can benefit a lot from the help of a computer proof assistant:\index{proof!ZhuShou@助手} it is wide and shallow with many short cases to consider, and a large part of the work consists of writing down what needs to be checked.)
...
\begin{eg}\label{ct:rezk-fundgpd-trunc1}
  ...
  \indexdef{JiChuDe@基础的!QunPei@群胚}%
  \index{JiChuDe@基础的!pregroupoid}%
  \indexsee{QunPei@群胚!JiChuDe@基础的}{fundamental group\-oid}%
...
\index{TongGou@同构!invariance under}%
...
\index{WanBeiDe@完备的!SegalKongJian@Segal空间}%
\index{Segal!space}%
...
\index{.WuQiong1TuoPuSi@$(\infty,1)$-拓扑斯}%
a strict category corresponds to an internal category (in the traditional sense) in the corresponding 1-topos\index{TuoPuSi@拓扑斯} of sheaves, while a category corresponds to a \emph{stack}.
\index{stack}%
...
However, in higher topos models of homotopy type theory, the Rezk completion corresponds to \emph{stack completion},\index{WanBei@完备!stack}\index{stack!WanBei@完备} which can be constructed either with a transfinite induction~\cite{jt:strong-stacks} or using a Yoneda embedding \cite{bunge:stacks-morita-internal}.
...
\begin{ex}\label{ex:slice-precategory}
  ...
  \indexsee{precategory!slice}{范畴, 切片的}%
  \indexsee{slice (pre)category}{范畴, 切片的}%
  Show that if $A$ is a category, so is $A/a$.
  \indexdef{FanChou@范畴!slice}%
\end{ex}
...
\begin{ex}\label{ex:functor-equiv-right-adjoint}
  \index{BanSui@伴随!HanZi@函子}%
  \index{BanSui@伴随!equivalence}%
...
\begin{ex}\label{ex:rezk-vankampen}
  \index{van KampenDingLi@van Kampen定理}%
  \index{DingLi@定理!van Kampen}%
  \index{JiChuDe@基础的!QunPei@群胚}%
  \index{JiChuDe@基础的!pregroupoid}%
