...
Categorically, we may consider \emph{discrete} groupoids, which are determined by a set of objects and only identity morphisms as higher morphisms; while topologically, we may consider spaces having the discrete topology.\index{LiSanDe@离散的!space}
...
\begin{thm}\label{thm:not-dneg}
  \index{ShuangChongFouDingLv@双重否定律}%
...
\begin{proof}
  ...
  From this, and a fixed-point-free autoequivalence\index{ZiDongTai@自同态!WuBuDongDianDe@无不动点的}, we will be able to extract a contradiction.
...
\begin{rmk}
  \index{XuanZeSuanZi@选择算子}%
  \indexsee{operator!XuanZe@选择}{选择算子}%
...
\index{LuoJi@逻辑!of mere propositions|(}%
...
In classical\index{mathematics!JingDianDe@经典的} mathematics, at least, these are the only two possibilities.
...
\indexsee{GongLi@公理!excluded middle}{excluded middle}%
\indexsee{Lv@律!PaiZhongDe@排中的}{excluded middle}%
...
\indexdef{ShuangChongFouDingLv@双重否定律}%
\indexdef{GongLi@公理!ShuangChongFouDing@双重否定}%
\indexdef{Lv@律!ShuangChongFouDingDe@双重否定的}%
is
\begin{equation}
  \label{eq:ldn}
  % \mathsf{DN}\;\defeq\;
...
For instance, in classical\index{mathematics!JingDianDe@经典的} mathematics, double negations are frequently used unnecessarily.
...
This sort of observation is even more pronounced in \emph{homotopy} type theory, where the powerful tools of univalence and higher inductive types allow us to constructively attack many problems that traditionally would require classical\index{mathematics!JingDianDe@经典的} reasoning.
...
\begin{defn}\label{defn:decidable-equality}
  ...
  \item A type $A$ is called \define{decidable}
    \indexdef{KePanDingDe@可判定的!LeiXing@类型}%
    \indexdef{LeiXing@类型!KePanDingDe@可判定的}%
    if $A+\neg A$.
  \item Similarly, a type family $B:A\to \type$ is \define{decidable}
    \indexdef{KePanDingDe@可判定的!LeiXingZu@类型族}%
    \indexdef{LeiXing@类型!family of!KePanDingDe@可判定的}%
    if \narrowequation{\prd{a:A} (B(a)+\neg B(a)).} \label{item:decidable-equality2}
  \item In particular, $A$ has \define{decidable equality}
    \indexdef{KePanDingDe@可判定的!equality}%
    \indexsee{equality!KePanDingDe@可判定的}{可判定相等性}%
...
\end{align}
The map~\eqref{eq:set-up} cannot be an equivalence, since then we could reproduce the paradoxes\index{BeiLun@悖论} of self-reference that are familiar from Cantorian set theory.
...
\begin{axiom}[Propositional resizing]
  \indexsee{GongLi@公理!propositional resizing}{propositional resizing}%
...
Thus, a classical\index{mathematics!JingDianDe@经典的} mathematician is accustomed to manipulating these two kinds of objects separately.
...
\indexsee{LeiXing@类型!squash}{truncation, propositional}%
\indexsee{squash type}{truncation, propositional}%
\indexsee{KuoHaoLeiXing@括号类型}{truncation, propositional}%
\indexsee{LeiXing@类型!KuoHao@括号}{truncation, propositional}%
...
\index{DiGuiFa@递归法!for truncation}%
...
\begin{defn} \label{defn:logical-notation}
  ...
  \index{quantifier!existential}%
  \indexsee{FanDe@泛的!quantifier}{quantifier, universal}%
  \index{quantifier!FanDe@泛的}%
...
\index{GongLi@公理!XuanZe@选择|(defstyle}%
\index{denial|(}%
...
This corresponds to a well-known equivalent form of the classical\index{mathematics!JingDianDe@经典的} axiom of choice, namely ``the cartesian product of a family of nonempty sets is nonempty''.
...
\index{unique!XuanZe@选择|(defstyle}%
\indexsee{GongLi@公理!XuanZe@选择!unique}{unique choice}%
...
Set-theoretically trained mathematicians often feel a twinge of guilt at such ``abuses of language''.\index{LanYong@滥用!of language}
...
As an experiment in exposition,  in what follows we will occasionally use \emph{adverbs}\index{FuCi@副词} to denote the application of such ``modalities'' as propositional truncation.
...
Note that this is a \emph{definition}\index{DingYi@定义!of adverbs} of the adverb ``merely'' as it is to be used in our informal mathematical English, in the same way that we define nouns\index{noun} like ``group'' and ``ring'', and adjectives\index{XingRuCi@形如词} like ``regular'' and ``normal'', to have precise mathematical meanings.
...
\item Using truncated logic as the default in type theory suffers from the same sort of ``abuse of language''\index{LanYong@滥用!of language} problems as set-theoretic foundations, which untruncated logic avoids.
...
\index{LeiXing@类型!KeSuoDe@可缩的|(defstyle}%
\index{KeSuoDe@可缩的!LeiXing@类型|(defstyle}%
...
\begin{defn}\label{defn:contractible}
  ...
  \indexdef{LeiXing@类型!singleton}%
  \indexsee{singleton type}{类型, 独元的}%
...
\index{LeiXing@类型!KeSuoDe@可缩的|)}%
\index{KeSuoDe@可缩的!LeiXing@类型|)}%
...
In fact, he defined the entire hierarchy of $n$-types by induction, as we will do in \cref{cha:hlevels}.\index{n-type@$n$-type!LeiXingLunZhongKeDingYiDe@类型论中可定义的}%

\cref{thm:not-dneg,thm:not-lem} rely in essence on a classical theorem of Hedberg,
\index{HedbergDingLi@Hedberg定理}%
\index{DingLi@定理!Hedberg's}%
...
\index{GongLi@公理!of reducibility}\index{resizing}%
...
This is the approach taken in logic enriched type theory~\cite{aczel2002collection} and in some presentations of the internal languages of toposes\index{TuoPuSi@拓扑斯} and related categories (e.g.~\cite{jacobs1999categorical,elephant}), as well as in the proof assistant \Coq.\index{proof!ZhuShou@助手!Coq@\textsc{Coq}}
...
Thus, the univalence axiom makes our type theory behave more like the internal logic of a topos;~see also \cref{cha:set-math}.\index{TuoPuSi@拓扑斯}
...
\begin{ex}\label{ex:prop-trunc-ind}
  \index{GuiNaFa@归纳法!for truncation}%
...
\begin{ex}\label{ex:n-set}
  Simplify the proof of \cref{thm:path-nat} by first proving that $\code(m,n)$ is a mere proposition for all $m,n$.
\end{ex}
