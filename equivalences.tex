...
\item bi-invertible maps,
  \index{HanShu@函数!ShuangKeNiDe@双可逆的}
  and
...
Thinking of $A$ as a higher groupoid, an inhabitant of $\prd{x:A}(x=x)$ is a natural transformation\index{ZiRanDe@自然的!transformation} from the identity functor of $A$ to itself.
Such transformations are said to form the \define{center of a category},
\index{center!of a category}%
\index{FanChou@范畴!center of}%
...
More generally, \cref{lem:autohtpy} implies that any ``Eilenberg--Mac Lane space'' $K(G,1)$, where $G$ is a nontrivial abelian\index{Qun@群!AbelDe@Abel的} group, will provide a counterexample; see \cref{cha:homotopy}.
...
\index{BanSui@伴随!equivalence!of types, half|(defstyle}%
...
\begin{defn}\label{defn:homotopy-fiber}
  The \define{fiber}
  \indexdef{fiber}%
  \indexsee{HanShu@函数!fiber of}{fiber}%
...
\begin{defn}\label{defn:linv-rinv}
  ...
  \indexdef{HanShu@函数!left invertible}%
  \indexdef{HanShu@函数!right invertible}%
...
\index{BanSui@伴随!equivalence!of types, half|)}%

\section{Bi-invertible maps}
\label{sec:biinv}

\index{HanShu@函数!ShuangKeNiDe@双可逆的|(defstyle}%
\index{ShuangKeNiHanShu@双可逆函数|(defstyle}%
...
\index{HanShu@函数!ShuangKeNiDe@双可逆的|)}%
\index{ShuangKeNiHanShu@双可逆函数|)}%
\index{equivalence!as bi-invertible function|)}%

\section{Contractible fibers}
\label{sec:contrf}

\index{HanShu@函数!KeSuoDe@可缩的|(defstyle}%
\index{KeSuoDe@可缩的!HanShu@函数|(defstyle}%
...
\index{HanShu@函数!KeSuoDe@可缩的|)}%
\index{KeSuoDe@可缩的!HanShu@函数|)}%
...
\indexdef{TongGou@同构!of sets}%
or a \define{bijection}.
\indexdef{ShuangShe@双射}%
\indexsee{HanShu@函数!ShuangSheDe@双射的}{双射}%
...
\begin{defn}\label{defn:surj-emb}
  ...
  \item We say $f$ is \define{surjective}
    \indexsee{surjective!HanShu@函数}{function, surjective}%
    \indexdef{HanShu@函数!surjective}%
...
\indexsee{surjective!HanShu@函数!split}{function, split surjective}%
\indexdef{HanShu@函数!split surjective}%
...
\begin{thm}[The 2-out-of-3 property]\label{thm:two-out-of-three}
  \index{ZhiErTuiSanXingZhi@知二推三性质}%
...
\index{retract!HanShuDe@函数的|(defstyle}%
...
\indexsee{HanShu@函数!fiberwise}{fiberwise transformation}%
\index{fiberwise!transformation|(defstyle}%
\indexsee{fiberwise!YingShe@映射}{fiberwise transformation}%
\indexsee{YingShe@映射!fiberwise}{fiberwise transformation}
...
\index{HanShuWaiYanXing@函数外延性!proof from univalence}%
...
\indexdef{HanShuWaiYanXing@函数外延性!weak}%
...
Thus, without assuming function extensionality, we can conclude that~\eqref{eq:uatofesp} is a retract\index{retract!LeiXingDe@类型的} of~\eqref{eq:uatofeps}.
...
The basic theorems about adjoint equivalences\index{BanSui@伴随!equivalence} such as \cref{lem:coh-equiv,thm:equiv-iso-adj} are adaptations of standard facts in higher category theory and homotopy theory.
...
\begin{ex}\label{ex:two-sided-adjoint-equivalences}
  Consider the type of ``two-sided adjoint equivalence\index{BanSui@伴随!equivalence} data'' for $f:A\to B$,
...
\begin{ex}[The unstable octahedral axiom]\label{ex:unstable-octahedron}
  \index{GongLi@公理!unstable octahedral}%
...
\begin{ex}\label{ex:2-out-of-6}
  \index{ZhiErTuiLiuXingZhi@知二推六性质}%
...
\begin{ex}\label{ex:funext-from-nondep}
  ...
  \indexdef{HanShuWaiYanXing@函数外延性!non-dependent}%
