...
\cref{sec:basics-sets}, is quite different from the sets of Zermelo--Fraenkel\index{JiHeLun@集合论!Zermelo--Fraenkel} set theory, which form a
...
(\cref{subsec:prop-subsets}) it is an elementary topos,\index{TuoPuSi@拓扑斯} and if we assume \LEM{} and \choice{} then it is a model of Lawvere's \emph{Elementary Theory of the Category of Sets}\index{Lawvere}.
\index{JiHeFanChouChuDengLiLun@集合范畴初等理论}%
...
\index{DaiShuJiHeLun@代数集合论}%
\index{JiHeLun@集合论!DaiShuDe@代数的}%
...
\index{FanChou@范畴!WanBeiDe@完备的}%
\index{WanBeiDe@完备的!FanChou@范畴}%
...
\index{FanChou@范畴!YuWanBei@余完备}%
\index{YuWanBeiFanChou@余完备范畴}%
...
\indexdef{FanChou@范畴!regular}%
\indexdef{regular!FanChou@范畴}%
%
\begin{enumerate}
...
\indexsee{YuDengHuaZi@余等化子!of sets}{set-coequalizer}%
It is convenient to express its universal property as follows.

\begin{lem}
\index{FanDe@泛的!property!of set-coequalizer}%
...
\end{equation*}
in $\uset$ defining the mapping cone\index{Zhui@锥!HanShuDe@函数的}. Then the type $C_f$ is contractible.
...
form the kernel\index{He@核!YouXuDui@有序对} pair of $c_R$.
...
\index{mathematics!JingDianDe@经典的}%
mathematics.
...
\index{setoid}\index{WanBei@完备!exact}%
...
Moreover, in topos theory a well-known sufficient condition for \LEM{} is the axiom of choice, which is of course often assumed as an axiom in classical\index{mathematics!JingDianDe@经典的} set theory.
...
\begin{thm}[Diaconescu]\label{thm:1surj_to_surj_to_pem}
  ...
  \index{DiaconescuDingLi@Diaconescu定理}\index{DingLi@定理!Diaconescu's}%
...
\begin{thm}\label{thm:ETCS}
  \index{JiHeFanChouChuDengLiLun@集合范畴初等理论}%
  \index{FanChou@范畴!well-pointed}%
  If the axiom of choice holds then the category $\uset$ is a well-pointed boolean\index{topos!BuErZhi@布尔值}\index{BuErZhi@布尔值!topos} elementary topos\index{TuoPuSi@拓扑斯} with choice.
\end{thm}
...
\begin{defn}\label{defn:card}
  ...
  \indexdef{LeiXing@类型!of cardinal numbers}%
  \indexdef{JiShu@基数}%
  \indexsee{number!JiShuDe@基数的}{基数}%
...
As usual for truncations, if $A$ is a set, then $\cd{A}$ denotes its image under the canonical projection $\set \to \trunc0\set \jdeq \card$; we call $\cd{A}$ the \define{cardinality}\indexdef{JiShuXing@技术性} of $A$.
...
\begin{defn}
  ...
  \indexdef{JiaFa@加法!of cardinal numbers}%
  \index{JiShu@基数!...DeJiaFa@...的加法}%
...
\begin{defn}
  ...
  \index{JiShu@基数!multiplication of}%
...
\begin{defn}
  ...
  \index{JiShu@基数!exponentiation of}%
...
\begin{thm}[Schroeder--Bernstein]
  \index{DingLi@定理!Schroeder--Bernstein}%
  \index{Schroeder--BernsteinDingLi@Schroeder--Bernstein定理}%
...
\begin{thm}[Cantor]
  \index{CantorDingLi@Cantor定理}%
  \index{DingLi@定理!Cantor's}%
...
\begin{defn}\label{defn:accessibility}
  ...
  \indexdef{KeDaXing@可达性}%
  \indexsee{KeDaDe@可达的}{可达性}%
  by $<$:
  \begin{itemize}
...
\index{GuiNaFa@归纳法!for accessibility}%
...
\begin{lem}
  Accessibility\index{KeDaXing@可达性} is a mere property.
\end{lem}
\begin{proof}
...
This is called \define{well-founded induction}\indexdef{well-founded!GuiNaFa@归纳法}.
...
In classical\index{mathematics!JingDianDe@经典的} logic, well-foundedness has a more well-known reformulation.
...
\index{GongLi@公理!of extensionality}%
...
\begin{proof}
  ...
  \index{ZiDongTai@自同态!of extensional well-founded relations}%
...
\begin{defn}\label{def:simulation}
  ...
  \indexdef{simulation}%
  \indexsee{HanShu@函数!simulation}{simulation}%
...
\begin{defn}
  ...
  \indexdef{simulation!ShouXianDe@受限的}%
  \indexdef{ShouXianDe@受限的!simulation}%
...
\noindent
More precisely, this theorem says that the type $\ord_{\UU_i}$ of ordinals in one universe\index{YuZhouCengJi@宇宙层级} is itself an ordinal in the next higher universe, i.e.\ $(\ord_{\UU_i},<):\ord_{\UU_{i+1}}$.
...
This lemma illustrates a potential pitfall of the ``typically ambiguous''\index{DianXingQiYi@典型歧义} style of using \UU to denote an arbitrary, unspecified universe.
...
\index{denial|(}%
We now show the equivalence of our ordinals with the more familiar classical\index{mathematics!JingDianDe@经典的} well-orderings.
...
In classical\index{mathematics!JingDianDe@经典的} mathematics, the characterization of \cref{thm:wellorder} is taken as the definition of a \emph{well-ordering}, with the \emph{ordinals} being a canonical set of representatives of isomorphism classes for well-orderings.
...
\noindent
Of course,~\ref{item:wop1} is a standard classical\index{mathematics!JingDianDe@经典的} version of the axiom of choice; see \cref{ex:choice-function}.
...
\begin{rmk}
  If we had given the wrong proof of \cref{thm:ordsucc} or \cref{thm:ordunion}, then the resulting proof of \cref{thm:wop} would be invalid: there would be no way to consistently assign universe levels\index{YuZhouCengJi@宇宙层级}.
...
\begin{defn}\label{defn:V}
  ...
  \indexdef{LeiJiDe@累积的!hierarchy, set-theoretic}%
  \indexdef{hierarchy!LeiJiDe, JiHeLunDe@累积的, 集合论的}%
...
\index{GuiNaFa@归纳法!for cumulative hierarchy}%
...
\define{$V$-set}
\indexdef{JiHe@集合!in the cumulative hierarchy}%
...
\indexdef{Lei@类!small}%
\indexdef{small!Lei@类}%
...
\begin{defn}\label{def:TypeOfElements}
  ...
  \indexdef{LeiXing@类型!of members}%
...
\begin{thm}\label{thm:VisCST}
  \index{GongLi@公理!of set theory, for the cumulative hierarchy}%
  ...
  \item \emph{extensionality:}
    ...
    \item \emph{pairing:} for all $u, v:V$, the class $\{u, v\} \defeq \setof{ x | x = u \vee x = v}$ is a $V$-set.
      %
    \item \emph{infinity:}\index{GongLi@公理!of infinity}  there is a $v:V$ with $\emptyset\in v$ and $x\in v$ implies $x\cup \{x\}\in v$.
    %
  \item \emph{union:} for all $v:V$, the class $\cup v\defeq \setof{ x | \exis{u:V} x \in u \in v}$ is a $V$-set.
   ...
   %
     \item \emph{replacement:}\index{GongLi@公理!of replacement} given any $r : V \to V$ and $x : V$, the class
       %
       \begin{equation*}
       ...
       is a $V$-set.
  %
   \item \emph{separation:}\index{GongLi@公理!of separation}  given any $a : V$ and $\UU$-small $C : V \to \propU$, the class
     %
     \begin{equation*}
...
\index{GouZaoXingDe@构造性的!JiHeLun@集合论}%
with many of the standard axioms.
However, as far as we know, it lacks the \emph{strong collection}
\index{GongLi@公理!strong collection}%
\index{DuiXiangZongTi@对象总体!strong}%
\index{strong!DuiXiangZongTi@对象总体}%
and \emph{subset collection}
\index{GongLi@公理!subset collection}%
\index{DuiXiangZongTi@对象总体!subset}%
\index{subset!DuiXiangZongTi@对象总体}%
...
Definitions of well-foundedness in type theory by an induction principle, including the inductive predicate of accessibility\index{KeDaXing@可达性}, were studied in~\cite{Huet80,Paulson86,Nordstrom88}, although the idea dates back to Gentzen's proof of the consistency\index{YiZhiXing@一致性!SuanShuDe@算数的} of arithmetic~\cite{Gentzen36}.
...
\begin{ex}\label{ex:well-pointed}
  ...
  \indexdef{FanChou@范畴!well-pointed}%
...
\begin{ex}\label{ex:add-ordinals}
  \index{JiaFa@加法!of ordinal numbers}%
...
\begin{ex}\label{ex:strong-collection}
  ...
  \indexdef{GongLi@公理!strong collection}%
  \indexdef{DuiXiangZongTi@对象总体!strong}%
  \indexdef{strong!DuiXiangZongTi@对象总体}%
  has the form:
   \begin{multline*}
