...
for homotopy theory (such as Quillen model categories).\index{QuillenMoXingFanChou@Quillen模型范畴}
 On the one hand,
...
Kan\index{KanFuXing@Kan复形} simplicial sets\index{simplicial!sets},
...
$(\infty,1)$-toposes\index{.WuQiong1TuoPuSi@$(\infty,1)$-拓扑斯}.
...
inductives are available in a setting like Kan\index{KanFuXing@Kan复形} simplicial sets, type
...
\indexsee{ShuXueDeXingShiHua@数学的形式化}{mathematics, formalized}%
Third, our synthetic approach is very amenable to computer-checked
proofs in proof assistants\index{proof!ZhuShou@助手} such as \Coq and \Agda.
...
\index{TongLun@同伦!equivalence!topological}%
(continuous
...
\caption{Homotopy groups of spheres~\cite{wikipedia-groups}.\index{TongLun@同伦!Qun@群!of sphere}
...
the additive group of integers,\index{integers} and $\Z_{m}$ is the cyclic group\index{Qun@群!cyclic}\index{cyclic group} of order~$m$.
}
...
$n$-fold iterated loop space\index{HuanLuKongJian@环路空间!iterated} of a pointed type $(A,a)$ is defined
recursively by:
\begin{align*}
  \Omega^0(A,a)&=(A,a)\\
...
\end{align*}
%
This gives a \emph{space} (i.e.\ a type) of $n$-dimensional loops\index{HuanLu@环路!n-@$n$-}, which itself has
...
abelian\index{Qun@群!AbelDe@Abel的}, by the Eckmann--Hilton argument \index{Eckmann--HiltonLunZheng@Eckmann--Hilton论证} (\cref{thm:EckmannHilton}).
...
automorphism\index{ZiDongTai@自同态!of Z, successor@of $\Z$, successor} of the set $\Z$, so that, for example, $\lloop \ct \opp
...
\index{JiChuDe@基础的!Qun@群!of circle|(}
\section{\texorpdfstring{$\pi_1(S^1)$}{π₁(S¹)}}
\label{sec:pi1-s1-intro}

In this section, our goal is to show that $\id {\pi_1(\Sn ^1)} {\Z}$.
In fact, we will show that the loop space\index{HuanLuKongJian@环路空间} ${\Omega(\Sn ^1)}$ is equivalent to $\Z$.
...
Moreover, the fiber over $x_0$ is the loop space\index{HuanLuKongJian@环路空间} $\Omega(B,x_0)$ --- in type theory this is obvious by definition of the loop space.
...
\index{FanDe@泛的!cover}%
\index{cover!FanDe@泛的}%
\index{covering space!FanDe@泛的}%
...
\indexsee{JieMa@解码}{encode-decode method}%
...
\begin{proof}
  ...
  \indexdef{real numbers!TongLunDe@同伦的}%
...
structure, and if $n>1$ the group is abelian\index{Qun@群!AbelDe@Abel的}.
...
\begin{defn}\label{def:pointedmap}
  ...
  \indexdef{pointed!YingShe@映射}%
  \indexsee{HanShu@函数!pointed}{pointed map}%
...
We call this the \define{zero map}\indexdef{Ling@零!YingShe@映射}\indexdef{HanShu@函数!Ling@零} and sometimes write it as $0:X\to Y$.

Recall that every pointed type $(X,x_0)$ has a loop space\index{HuanLuKongJian@环路空间} $\Omega (X,x_0)$.
We now note that this operation is functorial on pointed maps.\index{HuanLuKongJian@环路空间!DeHanZiXing@的函子性}\index{HanZi@函子!HuanLuKongJian@环路空间}
...
We now observe that the types occurring in this sequence are the iterated loop spaces\index{HuanLuKongJian@环路空间!iterated} of the base
...
\indexdef{He@核}%
\indexdef{pointed!YingShe@映射!DeHe@的核}%
...
\begin{proof}
  ...
  \index{Eckmann--HiltonLunZheng@Eckmann--Hilton论证} (\cref{thm:EckmannHilton}).
...
The usual properties of exact sequences of abelian groups\index{Qun@群!AbelDe@Abel的!exact sequence of} can be proved as
...
\index{TongLun@同伦!Qun@群!of sphere}
...
\begin{lem}\label{lem:hopf-construction}
  ...
  \indexdef{Hopf!GouZao@构造}%
  over $\susp A$ with fiber $A$ and total space $A*A$.
\end{lem}

\subsection{The Hopf fibration}

\index{HopfXianWeiCong@Hopf纤维丛|(}
\indexsee{fibration!Hopf}{HopfXianWei@Hopf纤维}
...
\begin{proof}
  ...
  \index{FubiniYuJiXianDingLi@Fubini余极限定理}
...
\index{FreudenthalWeiXuanDingLi@Freudenthal纬悬定理|(}%
\index{DingLi@定理!FreudenthalWeiXuan@Freudenthal纬悬|(}%
...
\index{LeiXing@类型!n-sphere@$n$-sphere}%
...
\begin{rmk}
  ...
  \index{simplicial!He@核}%
  \index{He@核!simplicial}%
...
\begin{eg}\label{eg:clvk}
  ...
  \index{RongHeZiYouJi@融合自由积}%
  \index{ZiYou@自由!product!RongHeDe@融合的}%
...
\begin{eg}\label{eg:torus}
  ...
  Thus, $\pi_1(T^2)$ is the quotient of the free group on two generators\index{ShengChengYuan@生成元!QunDe@群的} (i.e., $\pi_1(B)$) by the relation $p \ct q \ct \opp p \ct \opp q = 1$.
  This clearly yields the free \emph{abelian}\index{Qun@群!AbelDe@Abel的} group on two generators, which is $\Z\times\Z$.
\end{eg}
...
\begin{eg}\label{eg:kg1}
  In particular, suppose given any presentation\index{presentation!QunDe@群的} of a (set-)group $G = \langle X \mid R \rangle$, with $X$ a set of generators and $R$ a set of words in these generators\index{ShengChengYuan@生成元!QunDe@群的}.
  ...
  \indexdef{Eilenberg-Mac LaneKongJian@Eilenberg-Mac Lane空间}%
\end{eg}
...
\index{.WuQiong1TuoPuSi@$(\infty,1)$-拓扑斯}%
\index{.WuQiong1TuoPuSi@$(\infty,1)$-拓扑斯!non-hypercomplete}%
...
\index{GongLi@公理!WhiteheadYuanLi@Whitehead原理|(}%
...
Since homotopy groups are truncations of loop spaces\index{HuanLuKongJian@环路空间}, rather than path spaces, we need to modify this theorem to speak about these instead. Recall the map $\Omega f$ from \cref{def:loopfunctor}.
...
\indexdef{HanShu@函数!.infinity-connected@$\infty$-connected}%
...
\indexdef{LeiXing@类型!.infinity-truncated@$\infty$-truncated}%
\indexdef{.infinity-truncated type@$\infty$-truncated type}
or \define{hypercomplete}%
\indexdef{LeiXing@类型!hypercomplete}%
...
\indexdef{GongLi@公理!WhiteheadYuanLi@Whitehead原理}%
...
\begin{eg}
  Suppose we have $B:\nat\to\type$ such that for each $n$, the type $B(n)$ contains an $n$-loop\index{HuanLu@环路!n-@$n$-} which is not equal to $n$-fold reflexivity, say $p_n:\Omega^n(B(n),b_n)$ with $p_n \neq \refl{b_n}^n$.
...
$k$ is in fact $2$, our calculation of $\pi_4(\Sn ^3)$ is constructive,\index{mathematics!GouZaoXingDe@构造性的}
...
\begin{thm}[Blakers--Massey theorem]\label{Blakers-Massey}
  ...
  \indexsee{Blakers--MasseyDingLi@Blakers--Massey定理}{theorem, Blakers--Massey}%
...
\index{Eilenberg-Mac LaneKongJian@Eilenberg-Mac Lane空间}
For any abelian\index{Qun@群!AbelDe@Abel的} group $G$ and positive integer $n$, there is an $n$-type
...
also the first ones to formalize\index{mathematics!XingShiHuaDe@形式化的} the proof using a computer proof
assistant.
\begin{itemize}
\item
...
\begin{ex}\label{ex:decidable-equality-susp}
  ...
  (It is an open question\index{KaiFang@开放!WenTi@问题} whether this is provable without the assumption of decidable equality.)
\end{ex}
