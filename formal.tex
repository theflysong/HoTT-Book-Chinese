...
\index{XingShiHuaDe@形式化的!LeiXingLun@类型论|(}%
\index{LeiXingLun@类型论!XingShiHuaDe@形式化的|(}%
\index{LeiXingLunDeGuiZe@类型论的规则|(}%
...
\end{itemize}
%
Even the logical consistency\index{YiZhiXing@一致性} of homotopy type theory, namely that in the empty context there is no term $a:\emptyt$, is not obvious: if we had erroneously
...
\index{ShangXiaWen@上下文!well-formed}%
...
\index{BianLiang@变量!YuTiHuan@与替换}%
as discussed in
...
\index{YuanShiDe@原始的!ChangDe@常的}%
\index{ChangDe@常的!YuanShiDe@原始的}%
$c,c',\dots$, \emph{defined constants}\index{ChangDe@常的!DingYiDe@定义的} $f,f',\dots$, and term forming
operations
%
\[
...
\index{DingYi@定义!TongGuoJieGouGuiNa@通过结构归纳}%
\index{structural!DiGui@递归}%
\index{DiGui@递归!structural}%
...
the computation rule\index{JiSuanGuiZe@计算规则!for function types}
%
\[
  (\lam{x} t)(u) \defeq t[u/x],
\]
%
and the rules which make it a \emph{congruence} with respect to application and $\lambda$-abstraction\index{lambdaChouXiang@$\lambda$-抽象}:
%
\begin{itemize}
...
typical ambiguity\index{DianXingQiYi@典型歧义}, as we can always switch to a larger universe, which however does not affect the validity of the judgment.
...
\index{structural!GuiZe@规则|(}%
\index{GuiZe@规则!structural|(}%
...
\indexdef{GuiZe@规则!of substitution}%
and
\define{weakening},
\indexdef{GuiZe@规则!of weakening}%
...
derivable.\footnote{Such rules are called \define{admissible}\indexdef{GuiZe@规则!RongXuDe@容许的}\indexsee{RongXuDe@容许的!GuiZe@规则}{rule, admissible}.}
...
\begin{itemize}
\item a \define{formation rule}, stating when the type former can be applied;\index{formation rule}\index{GuiZe@规则!formation}
\item some \define{introduction rules}, stating how to inhabit the type;\index{introduction rule}\index{GuiZe@规则!introduction}
\item \define{elimination rules}, or an induction principle, stating how to use an
  element of the type;
  \index{GuiNaFa@归纳法}\index{eliminator}
\item \define{computation rules}, which are judgmental equalities explaining what happens when elimination rules are applied to results of introduction rules;
  \index{JiSuanGuiZe@计算规则}
  \indexsee{GuiZe@规则!JiSuan@计算}{计算规则}
...
variables.
\index{BianLiang@变量!ShouXian@受限}%
...
Inference of implicit arguments, typical ambiguity\index{DianXingQiYi@典型歧义} (\cref{sec:universes}),
...
\cref{sec:syntax-informally}, and similar results hold for \cref{sec:syntax-more-formally}. Figuring out which of these still hold when we add the features from \cref{sec:hott-features} quickly leads to open questions,\index{KaiFang@开放!WenTi@问题} as discussed at the end of this section.
...
has its own notion of computation, namely the computation rule\index{JiSuanGuiZe@计算规则!for function types}: 
\[
  (\lam{x} t)(u) \defeq t[u/x].
\]
This rule, together with the defining equations for the defined constants form
\emph{rewriting rules}\index{rewriting rule}\index{GuiZe@规则!rewriting} that determine reduction steps for a rewriting 
...
Logical consistency\index{YiZhiXing@一致性} (of the system in \cref{sec:syntax-informally}) follows
...
Similarly, we have the \emph{canonicity}\indexdef{DianFanXing@典范性} property that if $a:\N$ in the empty
...
never simplify, breaking \cref{lem:normal-forms}. It is an open question\index{KaiFang@开放!WenTi@问题}
...
The consistency\index{YiZhiXing@一致性} of Martin-L\"{o}f type theory extended with univalence and higher
...
univalence, a model in Kan\index{KanFuXing@Kan复形} complexes due to Voevodsky \cite{klv:ssetmodel}, and
