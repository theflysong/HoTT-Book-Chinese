...
同伦类型论的核心创意是类型可以被视作同伦论中的空间,抑或是范畴论中的高阶群胚。
...
Since $[0,1]\times [0,1]$ is 2-dimensional, we also speak of $H$ as a 2-dimensional \emph{path between paths}.\index{DaoLu@道路!2-}
...
equivalence classes of loops\index{HuanLu@环路} at some point $x_0$ (where two loops $p$
...
H(1,t) = x_0$ for all $t$) form a group called the \emph{fundamental
  group}.\index{JiChuDe@基础的!Qun@群}  This group is an \emph{algebraic invariant} of a space, which
...
a natural notion of 3-dimensional \emph{homotopy between homotopies},\index{DaoLu@道路!3-}
...
(weak) \emph{$\infty$-groupoid}.  An $\infty$-groupoid\index{.WuQiongQunPei@$\infty$-群胚} consists of a
...
Lane's pentagon\index{WuBianXing, Mac Lane@五边形, Mac Lane}.  Weakness also creates non-trivial interactions between
levels.

Every topological space $X$ has a \emph{fundamental $\infty$-groupoid}
\index{.WuQiongQunPei@$\infty$-群胚!JiChuDe@基础的}
...
the fundamental $\infty$-groupoid construction is adjoint\index{BanSui@伴随!HanZi@函子} to the
...
adjunction preserves homotopy theory (this is called the \emph{homotopy
  hypothesis/theorem},
\index{hypothesis!TongLun@同伦}%
\index{TongLun@同伦!hypothesis}%
...
$\infty$-groupoid.\index{.WuQiongQunPei@$\infty$-群胚}
...
\indexdef{DaoLu@道路!2-}\indexsee{2-DaoLu@2-道路}{道路, 2-}%
or a \define{2-dimensional path}.
\index{dimension!of paths}%
\indexsee{2WeiLuJing@2维路径}{道路, 2-}\indexsee{DaoLu@道路!2WeiDe@2维的}{道路, 2-}%
Similarly, $\id[{(\id[{(\id[A]{x}{y})}]{p}{q})}]{r}{s}$ is the type of
\define{3-dimensional paths}
\indexdef{DaoLu@道路!3-}\indexsee{3-DaoLu@3-道路}{道路, 3-}\indexsee{3WeiDaoLu@3维道路}{道路, 3-}\indexsee{DaoLu@道路!3WeiDe@3维的}{道路, 3-}%
...
This reduction to the ``reflexivity case'' is analogous to the reduction to the ``base case'' and ``inductive step'' in an ordinary proof by induction on the natural numbers, and also to the ``left case'' and ``right case'' in a proof by case analysis on a disjoint union or disjunction.\index{GuiNaFa@归纳法!for identity type}%
...
(This may seem so obvious as to not be worth saying, but if we view a definition by recursion as an algorithm\index{SuanFa@算法} for calculating values of a sequence, then it is precisely the process of executing that algorithm.)
...
This induction principle endows each type with the structure of an $\infty$-groupoid\index{.WuQiongQunPei@$\infty$-群胚}, and each function between two types with the structure of an $\infty$-functor\index{.infinity-functor@$\infty$-functor} between two such groupoids.  This is interesting from a mathematical point of view, because it gives a new way to work with
...
\index{DaoLu@道路|(}%
\index{.WuQiongQunPei@$\infty$-群胚!structure of a type|(}%
...
\begin{lem}\label{lem:opp}
  ...
  \indexdef{DaoLu@道路!inverse}%
  \indexdef{inverse!of path}%
  \index{equality!symmetry of}%a
  \index{symmetry!XiangDengXingDe@相等性的}%
\end{lem}
...
\begin{lem}\label{lem:concat}
  ...
  \indexdef{DaoLu@道路!LianJie@连接}%
  \indexdef{DaoLu@道路!FuHe@复合}%
  \indexdef{DaoLuDeLianJie@道路的连接}%
  \indexdef{FuHe@复合!of paths}%
  \index{equality!transitivity of}%
  \index{transitivity!XiangDengXingDe@相等性的}%
\end{lem}
...
\index{mathematics!XingShiHuaDe@形式化的}%
...
\begin{lem}\label{thm:omg}%[The $\omega$-groupoid structure of types]
  \index{JieHeXing@结合性!of path concatenation}%
  \index{unit!DaoLuLianJieLv@道路连接律}%
...
\index{JieHeXing@结合性!of path concatenation!coherence of}%
\index{globular operad}%
\index{operad}%
\index{QunPei@群胚!GaoJieDe@高阶的}%
...
In addition to ordinary paths, we will use paths of paths (i.e.\ elements of a type $p =_{x=_A y} q$), which as remarked previously we call \emph{2-paths}\index{DaoLu@道路!2-} or \emph{2-dimensional paths}, and perhaps occasionally paths of paths of paths (i.e.\ elements of a type $r = _{p =_{x=_A y} q} s$), which we call \emph{3-paths}\index{DaoLu@道路!3-} or \emph{3-dimensional paths}.
It is possible to define a general notion of \emph{$n$-dimensional path}
\indexdef{DaoLu@道路!n-@$n$-}%
\indexsee{n-path@$n$-path}{path, $n$-}%
\indexsee{n-dimensional path@$n$-dimensional path}{path, $n$-}%
\indexsee{DaoLu@道路!n-dimensional@$n$-dimensional}{path, $n$-}%
...
In set theory, the proposition $a=a$ is entirely uninteresting, but in homotopy theory, paths from a point to itself are called \emph{loops}\index{HuanLu@环路} and carry lots of interesting higher structure.
Thus, given a type $A$ with a point $a:A$, we define its \define{loop space}
\index{HuanLuKongJian@环路空间}%
...
It can also be useful to consider the loop space\index{HuanLuKongJian@环路空间!iterated}\index{iterated loop space} \emph{of} the loop space of $A$, which is the space of 2-dimensional loops on the identity loop at $a$.
...
\begin{thm}[Eckmann--Hilton]\label{thm:EckmannHilton}
  ...
  \index{Eckmann--HiltonLunZheng@Eckmann--Hilton论证}%
\end{thm}
...
\indexdef{FuHe@复合!of paths!horizontal}%
by:
\[
...
The foregoing fact, which is known as the \emph{Eckmann--Hilton argument}, comes from classical homotopy theory, and indeed it is used in \cref{cha:homotopy} below to show that the higher homotopy groups of a type are always abelian\index{Qun@群!AbelDe@Abel的} groups.
...
\begin{defn} \label{def:pointedtype}
  ...
  \indexsee{pointed!LeiXing@类型}{类型, 定点的}%
  \indexdef{LeiXing@类型!pointed}%
...
\begin{defn} \label{def:loopspace}
  ...
  \indexdef{HuanLuKongJian@环路空间}%
  of $(A,a)$ to be the following pointed type:
  \[\Omega(A,a)\defeq ((\id[A]aa),\refl a).\]
  An element of it will be called a \define{loop}\indexdef{HuanLu@环路} at $a$.
  For $n:\N$, the \define{$n$-fold iterated loop space} $\Omega^{n}(A,a)$
  \indexdef{HuanLuKongJian@环路空间!iterated}%
  \indexsee{HuanLuKongJian@环路空间!n-fold@$n$-fold}{loop space, iterated}%
  ...
  \indexdef{HuanLu@环路!n-@$n$-}%
  \indexsee{n-loop@$n$-loop}{loop, $n$-}%
  or an \define{$n$-dimensional loop}
  \indexsee{HuanLu@环路!n-dimensional@$n$-dimensional}{loop, $n$-}%
...
\begin{lem}\label{lem:map}
  ...
  \indexdef{YingYong@应用!of function to a path}%
  \indexdef{DaoLu@道路!HanShuZai...DeYingYong@函数在...的应用}%
  \indexdef{HanShu@函数!...ZaiYiTiaoDaoLuShangDeYingYong@...在一条道路上的应用}%
  \indexdef{ZuoYong@作用!DaoLuShangHanShuDe@道路上函数的}%
\end{lem}
...
\begin{lem}[Path lifting property]\label{thm:path-lifting}
  \indexdef{DaoLu@道路!lifting}%
  \indexdef{lifting!DaoLu@道路}%
...
\begin{rmk}
  ...
  \indexdef{section!LeiXingZuDe@类型族的}%
...
\begin{lem}[Dependent map]\label{lem:mapdep}
  \indexdef{YingYong@应用!DaoLuShangYiZhiHanShuDe@道路上依值函数的}%
  \indexdef{DaoLu@道路!YiZhiHanShuZai...DeYingYong@依值函数在...的应用}%
  \indexdef{HanShu@函数!YiZhiDe@依值的!...ZaiYiTiaoDaoLuShangDeYingYong@...在一条道路上的应用}%
  \indexdef{ZuoYong@作用!of a dependent function on a path}%
...
\indexsee{YiZhiDe@依值的!DaoLu@道路}{道路, 依值的}%
\index{DaoLu@道路!YiZhiDe@依值的}%
...
Under the homotopical interpretation, this dependent function type consists of \emph{continuous} paths or \emph{functorial} equivalences, and thus may be regarded as the type of \emph{homotopies} or of \emph{natural isomorphisms}.\index{TongGou@同构!ZiRanDe@自然的}
...
\indexsee{TongLun@同伦!equivalence}{equivalence}%
...
(This is closely related to the observation in higher category theory that often one needs to consider \emph{adjoint} equivalences\index{BanSui@伴随!equivalence} rather than plain equivalences.)
...
\begin{defn}\label{defn:quasi-inverse}
  ...
  \indexsee{HanShu@函数!quasi-inverse of}{quasi-inverse}%
...
\begin{eg}\label{eg:idequiv}
  \index{identity!HanShu@函数}%
  \index{HanShu@函数!identity}%
...
\index{JiSuanGuiZe@计算规则!propositional!for identities between pairs}%
\begin{align*}
...
\index{LeiXing@类型!YiZhiHanShu@依值函数|(}%
...
\begin{axiom}[Function extensionality]\label{axiom:funext}
  \indexsee{GongLi@公理!HanShuWaiYanXing@函数外延性}{function extensionality}%
  \indexdef{HanShuWaiYanXing@函数外延性}%
...
From this point of view, $\happly$ is the \emph{elimination rule}, while the homotopies witnessing $\funext$ as quasi-inverse to $\happly$ become a propositional computation rule\index{JiSuanGuiZe@计算规则!propositional!for identities between functions}
\[
...
\index{DaoLu@道路!YiZhiDe@依值的!in function types}
...
\index{DaoLu@道路!YiZhiDe@依值的!in dependent function types}
...
\begin{axiom}[Univalence]\label{axiom:univalence}
  \indexdef{FanDengGongLi@泛等公理}%
  \indexsee{GongLi@公理!FanDengXing@泛等性}{univalence axiom}%
...
\indexdef{LeiXing@类型!YuZhou@宇宙!FanDengDe@泛等的}%
\indexdef{FanDengYuZhou@泛等宇宙}%
...
\item The elimination rule, which is $\idtoeqv$,
  ...
  \]
\item The propositional computation rule\index{JiSuanGuiZe@计算规则!propositional!for univalence},
  \[
...
\index{DaoLu@道路!YiZhiDe@依值的!in identity types}
...
\index{LeiXing@类型!negative}\index{negative!LeiXing@类型}%
\index{polarity}%
...
\index{LeiXing@类型!positive}\index{positive!LeiXing@类型}%
...
applies to the other.  Common ``abuses of notation''\index{LanYong@滥用!of notation} become formally
...
\index{JieHeXing@结合性!of semigroup operation}%
...
While the univalence axiom\index{FanDengGongLi@泛等公理} ensures that this map exists, we need to use
...
arrived at a standard definition of a \emph{semigroup isomorphism}:\index{TongGou@同构!semigroup} a
...
of isomorphism, by defining a \emph{semigroup homomorphism}\index{TongTai@同态!semigroup} to be a map
...
\begin{thm}\label{thm:prod-ump}
  \index{FanDe@泛的!property!of cartesian product}%
...
\begin{thm}\label{thm:ttac}
  \index{FanDe@泛的!property!YiZhiDuiDe@依值对的}%
...
(On the other hand, the classical\index{mathematics!JingDianDe@经典的} mathematician may find that~\eqref{eq:sigma-ump-map} does not carry the usual meaning of the axiom of choice, since we have already specified the values of $g$, and there are no choices left to be made.
...
the cartesian closure adjunction\index{BanSui@伴随!HanZi@函子}:
...
\index{LeiXing@类型!identity}%
\index{FanDe@泛的!property!of identity type}%
\begin{equation}
  \label{eq:path-lump}
...
\index{LeiXing@类型!limit}%
\index{LeiXing@类型!YuJiXian@余极限}%
\index{limit!of types}%
\index{YuJiXian@余极限!of types}%
...
\index{LeiXing@类型!unit}%
\indexsee{initial!LeiXing@类型}{类型, 空}%
\indexsee{terminal!LeiXing@类型}{类型, 单位}%
...
The actual homotopical interpretation, with identity types as path spaces, and type families as fibrations, is due to \cite{AW}, who used the formalism of Quillen model categories.  An interpretation in (strict) $\infty$-groupoids\index{.WuQiongQunPei@$\infty$-群胚} was also given in the thesis \cite{mw:thesis}.
...
Limits\index{limit!of types} of diagrams over directed graphs\index{Tu@图} are the easiest general sort of limit to formalize; the problem with diagrams over categories (or more generally $(\infty,1)$-categories)
\index{.infinity1-category@$(\infty,1)$-category}%
\indexsee{FanChou@范畴!.infinity1-@$(\infty,1)$-}{$(\infty,1)$-category}%
...
Resolving this problem is an important open question\index{KaiFang@开放!WenTi@问题} in homotopy type theory.
...
\begin{ex}\label{ex:npaths}
  Define, by induction on $n$, a general notion of \define{$n$-dimensional path}\index{DaoLu@道路!n-@$n$-} in a type $A$, simultaneously with the type of boundaries for such paths.
\end{ex}
...
\begin{ex}\label{ex:coprod-ump}
  \index{FanDe@泛的!property!YuJiDe@余集的}%
...
\begin{ex}\label{ex:sigma-assoc}
  ...
  \index{JieHeXing@结合性!of Sigma-types@of $\Sigma$-types}%
...
\begin{ex}\label{ex:pullback}
  ...
  \indexdef{KeJiaoHuanDe@可交换的!square}%
  \begin{equation*}
  \vcenter{\xymatrix{
      P\ar[r]^h\ar[d]_k &
      A\ar[d]^f\\
...
\begin{ex}\label{ex:sigma-eq-components-neq}
  We have seen that $\bfalse \neq \btrue$ (rmk:false-neq-true, 不知怎得, 这里的引用失效了). Show that, nevertheless, $(\bool, \bfalse) =_{\sm{A:\type}A} (\bool, \btrue)$ (see \cref{rmk:sigma-equality-extraction}).
\end{ex}
