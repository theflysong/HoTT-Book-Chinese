...
例如, 通过这种方式我们可以证明命题唯一性原理, 该原理表明 $A\times B$ 的每个元素都等于一个有序对。
...
归纳法使我们能够证明 $\unit$ 的命题唯一性原理, 该原理断言其唯一的居留元是 $\ttt$。
也就是说, 我们可以通过使用定义方程
...
\begin{align*}
  ...
  g_1 &: \prd{b:B} C(\inr(b)).
\end{align*}
This yields $f$ with the defining equations:\index{JiSuanGuiZe@计算规则!for coproduct type}
\begin{align*}
  f(\inl(a)) &\defeq g_0(a), \\
  f(\inr(b)) &\defeq g_1(b).
\end{align*}
...
and unit\index{LeiXing@类型!unit} types as $\unit + \unit$. However,
...
However, note that unlike in classical\index{mathematics!JingDianDe@经典的} mathematics, we do not use elements of $\bool$ as truth values
\index{Zhi@值!truth}%
or as propositions.
...
The elements of $\nat$ are constructed using $0 : \nat$\indexdef{Ling@零} and the successor\indexdef{successor} operation $\suc : \nat \to \nat$.
...
This gives rise to $f$ with the defining equations\index{JiSuanGuiZe@计算规则!for natural numbers}
\begin{align*}
  f(0) &\defeq c_0, \\
