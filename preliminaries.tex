...
\indexdef{BianLiang@变量!captured}%
...
\indexsee{nullary!product}{类型, 单位}%
...
Like other binding constructs such as $\lambda$-abstractions and $\Pi$s, $\Sigma$s automatically scope\index{ZuoYongYu@作用域} over the rest of the expression unless delimited, so e.g.\ $\sm{x:A} B(x) \to C$ means $\sm{x:A} (B(x) \to C)$.
...
\index{MoShiPiPei@漠视匹配|(defstyle}%
\indexsee{matching}{漠视匹配}%
\index{DingYi@定义!by pattern matching|(}%
...
\begin{ex}\label{ex:composition}
  ...
  \indexdef{composition!HanShuDe@函数的}%
  \indexdef{HanShu@函数!composition}%
 $g\circ f:A\to C$.
