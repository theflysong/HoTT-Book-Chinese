...
% DOCUMENT CLASS
\documentclass[\OPTfontsize]{ctexbook}
...
\indexsee{principle}{GongLi@公理}
\indexsee{number!ShiShuDe@实数的}{实数}
\indexsee{AbelQun@Abel群}{group, abelian}
\indexsee{sequence!Cauchy}{CauchyLie@Cauchy列}
\indexsee{BanSui@伴随}{伴随函子}
\indexsee{higher topos}{$(\infty,1)$-topos}
\indexsee{topos!higher}{$(\infty,1)$-topos}
\indexsee{source!HanShuDe@函数的}{定义域}
\indexsee{target!HanShuDe@函数的}{陪域}
\indexsee{LeiXing@类型!truncation of}{truncation}
\indexsee{propositional!truncation}{truncation}
\indexsee{proof-relevant mathematics}{mathematics, proof-relevant}
\indexsee{JingDianDe@经典的!mathematics}{mathematics, classical}
\indexsee{JingDianDe@经典的!logic}{logic}
\indexsee{GouZaoXingDe@构造性的!mathematics}{mathematics, constructive}
\indexsee{GouZaoXingDe@构造性的!logic}{logic}
\indexsee{intuitionistic logic}{logic}
\indexsee{DingYi@定义!GuiNaDe@归纳的}{类型, 归纳的}
\indexsee{GuiNaDe@归纳的!DingYi@定义}{类型, 归纳的}
\indexsee{ShouXianDe@受限的!totally}{totally bounded}
\indexsee{sum!of numbers}{加法}
\indexsee{LianXuYingShe@连续映射}{函数, 连续的}
\indexsee{HanShu@函数!continuity of@``continuity'' of}{``continuity''}
\indexsee{HanShu@函数!DeHanZiXing@的函子性}{``functoriality''}
\indexsee{BianMa@编码}{encode-decode method}
...
\indexsee{ShengChengYuan@生成元!of an inductive type}{构造子}
\indexsee{QunPei@群胚!.infinity-@$\infty$-}{$\infty$-groupoid}
\indexsee{higher groupoid}{$\infty$-groupoid}
\indexsee{hierarchy!of n-types@of $n$-types}{$n$-type}
\indexsee{TongLun@同伦!n-type@$n$-type}{$n$-type}
\indexsee{TongLun@同伦!theory, classical}{经典同伦论}
\indexsee{TongLun@同伦!fiber}{fiber}
\indexsee{TongLun@同伦!limit}{limit of types}
\indexsee{TongLun@同伦!YuJiXian@余极限}{类型的余极限}
\indexsee{implementation}{proof assistant}
\indexsee{notation, abuse of}{LanYongGaiNian@滥用概念}
\indexsee{language, abuse of}{LanYongYuYan@滥用语言}
\indexsee{operator!GuiNaFa@归纳法}{归纳法}
\indexsee{operator!modal}{modality}
\indexsee{KeJiaoHuanDe@可交换的!Qun@群}{group, abelian}
\indexsee{countable axiom of choice}{XuanZeGongLi, KeShuDe@选择公理, 可数的}
