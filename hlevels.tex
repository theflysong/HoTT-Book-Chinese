...
For instance, a space is $0$-connected (also called just ``connected'') if it has only one connected component, and $1$-connected (also called ``simply connected'') if it also has no nontrivial loops (though it may have nontrivial higher loops between loops\index{HuanLu@环路!n-@$n$-}).
...
\begin{defn}\label{def:hlevel}
  ...
  \indexsee{n-truncated@$n$-truncated!LeiXing@类型}{$n$-type}%
  \indexsee{LeiXing@类型!n-type@$n$-type}{$n$-type}%
  \indexsee{LeiXing@类型!n-truncated@$n$-truncated}{$n$-type}%
...
\begin{thm}\label{thm:h-set-uip-K}
 ...
 \indexdef{GongLi@公理!StreicherGongLiK@Streicher公理K}%
...
\indexdef{merely!KePanDingXiangDengXing@可判定相等性}%
\[ \prd{a,b:A} (\brck{a=b} + \neg\brck{a=b}). \]
...
\index{GongLi@公理!StreicherGongLiK@Streicher公理K!generalization to n-types@generalization to $n$-types}%
Although Hedberg's theorem appears rather special to sets ($0$-types), ``Axiom K'' generalizes naturally to $n$-types.
Note that the ordinary Axiom K (as a property of a type $X$) states that for all $x:X$, the loop space\index{HuanLuKongJian@环路空间} $\Omega(X,x)$ (see \cref{def:loopspace}) is contractible.
...
\begin{thm}\label{thm:ntype-nloop}
  \index{HuanLuKongJian@环路空间!iterated}%
...
0-truncation.
We now explain a better generalization of this, which truncates any type into an $n$-type for any $n\geq -2$; in classical homotopy theory this would be called its \define{$n^{\mathrm{th}}$ Postnikov section}.\index{PostnikovTa@Postnikov塔}
...
\begin{lem}[Universal property of truncations]\label{thm:trunc-reflective}
  \index{FanDe@泛的!property!of truncation}%
...
\begin{thm}
  ...
  \index{FanDe@泛的!property!of pushout}%
...
\indexdef{HanShu@函数!n-connected@$n$-connected}%
\indexsee{n-connected@$n$-connected!HanShu@函数}{function, $n$-connected}%
...
\indexsee{n-connected@$n$-connected!LeiXing@类型}{type, $n$-connected}%
\indexdef{LeiXing@类型!n-connected@$n$-connected}%
 if the unique function $A\to\unit$ is $n$-connected, i.e.\ if $\trunc nA$ is contractible.
\end{defn}
\indexsee{LianTongDe@连通的!HanShu@函数}{function, $n$-connected}
...
\indexdef{LianTongDe@连通的!LeiXing@类型}%
\indexdef{LeiXing@类型!LianTongDe@连通的}%
and when it is $1$-connected we say it is \define{simply connected}.
\indexdef{simply connected type}%
\indexdef{LeiXing@类型!simply connected}%
...
\index{HanShu@函数!n-connected@$n$-connected}

\begin{lem}
\index{retract!HanShuDe@函数的}%
...
Importantly, $n$-connected functions can be equivalently characterized as those which satisfy an ``induction principle'' with respect to $n$-types.\index{GuiNaFa@归纳法!for connected maps}
...
\begin{defn}
  ...
  \indexdef{n-truncated@$n$-truncated!HanShu@函数}%
  \indexdef{HanShu@函数!n-truncated@$n$-truncated}%
...
\indexdef{HanShu@函数!n-image of@$n$-image of}%
of $f$ is defined as
\begin{equation*}
...
\index{DiGuiFa@递归法!for a modality}%
\index{GuiNaFa@归纳法!for a modality}%
...
\indexdef{modal!LeiXing@类型}%
\indexdef{LeiXing@类型!modal}%
...
\index{FanDe@泛的!property!of a modality}%
...
As mentioned in \cref{subsec:when-trunc}, we may in general use adverbs\index{FuCi@副词} to speak informally about such modalities, such as ``merely''\index{merely} for the propositional truncation and ``purely''\index{purely} for the identity modality
...
\indexdef{HanShu@函数!.circle-connected@$\modal$-connected}%
...
\indexdef{HanShu@函数!.circle-truncated@$\modal$-truncated}%
...
These are a categorification of ``Lawvere-Tierney\index{Lawvere}\index{Tierney} topologies'' in elementary topos\index{TuoPuSi@拓扑斯} theory,
...
\begin{ex}\label{ex:s2-colim-unit}
  ...
  Define a \define{graph}\indexdef{Tu@图} $\Gamma$ to consist of a type $\Gamma_0$ and a family $\Gamma_1 : \Gamma_0 \to \Gamma_0 \to \UU$.
  ...
  The \define{colimit}\index{YuJiXian@余极限!of types} of such a diagram is the higher inductive type $\colim(F)$ generated by
  \begin{itemize}
...
\begin{ex}\label{ex:acnm}
  \indexdef{GongLi@公理!XuanZe@选择!ACnm@$\choice{n,m}$}%
  ...
  \item Of course, $\choice{n,m}\Rightarrow \choice{k,m}$ if $k\le n$.
    ...
    (These are open questions.)\index{KaiFang@开放!WenTi@问题}
  \end{enumerate}
\end{ex}
...
\begin{ex}\label{ex:acconn}
  ...
  \indexdef{n-connected@$n$-connected!XuanZeGongLi@选择公理}%
  \indexdef{GongLi@公理!XuanZe@选择!n-connected@$n$-connected}%
  to be the statement
  \begin{quote}
  ...
  \item Are there any other implications between the $n$-connected axioms of choice and the principles $\choice{n,m}$?
    (This is an open question.)\index{KaiFang@开放!WenTi@问题}
  \end{enumerate}
\end{ex}
...
\begin{ex}\label{ex:prop-modalities}
  ...
  \item Show that $X\mapsto P*X$ is a left exact modality, where $*$ denotes the join (see \cref{sec:colimits}).
    ...
    \indexdef{BiDe@闭的!modality}%
    \indexdef{modality!BiDe@闭的}%
    associated to $P$.
  \end{enumerate}
\end{ex}
...
\begin{ex}\label{ex:f-local-type}
  ...
  \indexdef{LeiXing@类型!f-local@$f$-local}%
...
\begin{ex}\label{ex:s2-colim-unit-2}
  ...
  Define a \define{graph with composition}\indexdef{Tu@图!with composition} $\Gamma$ to be a graph as in \cref{ex:s2-colim-unit} together with for each $x,y,z:\Gamma_0$, a function $\Gamma_1(y,z) \to \Gamma_1(x,y) \to \Gamma_1(x,z)$, written as $\delta\mapsto\gamma \mapsto \delta \circ \gamma$.
...
\begin{ex}\label{ex:categorical-connectedness}
  ...
  \indexdef{LianTongDe@连通的!FanChouDi@范畴地} if for every types $B, C$ the canonical map
